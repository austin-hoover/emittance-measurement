...
for node in controller.sequence.getNodes():
    ...
    file.write(
        "{} {} {} {} {} {} {}\n".format(
            node.getId(), mu_x, mu_y, alpha_x, alpha_y, beta_x, beta_y
        )
    )
file.close()
...
def print_progress_report(controller):
    ...
    print(
        "Max betas (BPM17 - WS24) = {:.2f}, {:.2f}".format(
            *controller.max_betas(start="RTBT_Diag:BPM17", stop="RTBT_Diag:WS24")
        )
    )
    print(
        "Max betas (WS24 - Target) =  {:.2f}, {:.2f}".format(
            *controller.max_betas(start="RTBT_Diag:WS24", stop="RTBT:Tgt")
        )
    )
...
muxx = optics.lin_phase_range(
    mux0 + dmux_min, mux0 + dmux_max, steps_per_dim, endpoint=False
)
muyy = optics.lin_phase_range(
    muy0 + dmuy_min, muy0 + dmuy_max, steps_per_dim, endpoint=False
)
...
for i, mux in enumerate(muxx):
    for j, muy in enumerate(muyy):
        ...
        for quad_id, field, default_field in zip(quad_ids, fields, default_fields):
            ...
            print(
                "  {}: {:.3f} {:.3f} {:.3f} {:.3f}".format(
                    quad_id, field, default_field, abs_diff, abs_frac_diff
                )
            )
        ...
        file.write(
            "node_id M11 M12 M13 M14 M21 M22 M23 M34 M31 M32 M33 M34 M41 M42 M43 M44\n"
        )
        for node in controller.sequence.getNodes():
